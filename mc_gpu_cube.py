...
def simul_gpu(x_0, N, n, d, 
    ...
    assert d==cube.shape[1], "The dim. of X and the dim. of the cube do not coincide"
    ...
    if FIG: 
        ...
        for s, e in combinations(cp.array(list(product(cube[:,0], cube[:,1], cube[:,2]))), 2):
    ...
    for i in range(n):
        ...
        idx_tmp = cp.logical_and(X[tau]>cube[0], X[tau]<cube[1])
        ...
        dist = cp.concatenate((cp.square(X[tau1]-cube[0]), cp.square(X[tau1]-cube[1])), axis=1)
...
def minimize(x_0, N, n, d, iter_max,
    ...
    outside = cp.logical_and(x-cube[0], x<cube[1])
    outside = cp.sum(outside) < 3
    # outside = cp.square(x - sphere_c).sum() > sphere_r**2
    assert cp.logical_not(outside), "ERROR: x_0 is outside the region, change point"
    ...
    while err>err_min:
        ...
        if outside:   
            x = cp.copy(x_tm1)
            print("The GD went outside the region, we go back one step in time")
...
if __name__ == '__main__':
    ...
    # x_0 = cp.array([-31.75191066,  81.19564517,  23.77996511])
    x_0 = cp.array([36., 17., 23.])
    N = 10000
    n = 200
    iter_max = 5e1
    d = 3
    # square = [[x1, x2, x3],[X1, X2, X3]]
    cube = cp.array([[-100., -100., -100.], [100., 100., 100.]])
    ...
    # ax.axis([-101, 101, -101, 101])
    # ax.axis([10, 30, 50, 65])
    # plt.gca().add_patch(plt.Circle((0, 0), (sphere_r[0]**2-plane_z**2)**0.5, fill=False, color='g'))
    # rect = Rectangle((cube_cpu[0,:2]), cube_cpu[1,0]-cube_cpu[0,0], cube_cpu[1,1]-cube_cpu[0,1], fill=False, color='g')
    plt.gca().add_patch(Rectangle((cube_cpu[0,:2]), cube_cpu[1,0]-cube_cpu[0,0], cube_cpu[1,1]-cube_cpu[0,1], fill=False, color='g'))
    plt.pause(1e-7)
    ...
    while True:
        ...
        arrow_dir = ax.arrow(x_cpu[0], x_cpu[1], 
                            ...
                            color='g')
        tmp = cp.linspace(cube[0,0], cube[1,0], 5000).reshape(-1,1)
        line = -direction[0]*tmp[:, 0] + ineq_value
        line /= direction[1]
        idx_tmp = cp.logical_and(cube[0,1] <= line, line <= cube[1,1])
        line = line[idx_tmp]
        tmp = tmp[idx_tmp]
        ...
        if iteration>=minim_iter+1:
            if cp.min(cp.linalg.norm(x-points[:-minim_iter],axis=1))<step_exploration:
                break
    np.save('points_total_cube.npy', points_total.get())
    np.save('points_cube.npy', points.get())
